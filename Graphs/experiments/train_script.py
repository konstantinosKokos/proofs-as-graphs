...
def init_train(which: str) -> Tuple[Tuple[DataLoader, DataLoader, DataLoader], PairClassifier, TrainLogger]:
    ...
    train_dl = pair_loader([graphs_to_data(*sample) for sample in train], 64, True)
    dev_dl = pair_loader([graphs_to_data(*sample) for sample in dev], 512, False)
    test_dl = pair_loader([graphs_to_data(*sample) for sample in test], 512, False)
    logger = TrainLogger(CrossEntropyLoss(ignore_index=-100, reduction='mean',
                                          weight=torch.tensor([1.24, 0.58, 2.06], device='cuda')),
                         Scheduler(AdamW(model.parameters(), lr=0, weight_decay=1e-02),
                                   make_cyclic_triangular_schedule(max_lr=3e-04, warmup_steps=len(train_dl),
