...
class Encoder(Module):
    def __init__(self, tokenizer: Tokenizer, device: str, hidden_size: Maybe[int] = None):
        ...
        if tokenizer.name in {'bert', 'robert'}:
            ...
            self.core = GRUWrapper(tokenizer, device, hidden_size, self.pad_value)
        else:
            raise ValueError
...
class GRUWrapper(Module):
    def __init__(self, tokenizer: Tokenizer, device: str, hidden_size: Maybe[int], pad_value: int):
        ...
        self.rnn = GRU(input_size=tokenizer.word_tokenizer.dim, hidden_size=hidden_size, bidirectional=True,
                       num_layers=2, batch_first=True, dropout=0.5).to(device)

    def forward(self, word_ids: Tensor) -> Tensor:
        ctx, _ = self.rnn(self.embedding(word_ids))
        return sum(ctx.chunk(2, dim=-1))


class BertWrapper(Module):
    def __init__(self, tokenizer: Tokenizer, freeze: bool, device: str):
